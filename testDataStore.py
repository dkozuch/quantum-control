...
class test_DataStore(unittest.TestCase):
    ...
    def test_withInput(self):
        ...
        np.testing.assert_array_equal(data.path_desired, txy_desired[:,1:])
        self.assertEqual(data.path_obs.shape, txy_desired[:,1:].shape)
    ...
    def test_timeOrder(self):
        ...
        txy_desired2 = np.array([ [0,1,2,4,3], np.arange(1,6), np.arange(11,16) ] ).T
    ...
    def test_hasNanInf(self):
        ...
        txy_desired = np.array([[0.], [np.nan], [1.]])
        self.assertRaises(ValueError, DataStore, txy_desired)
        txy_desired2 = np.array([[1.], [1.], [np.inf]])
        self.assertRaises(ValueError, DataStore, txy_desired2)
